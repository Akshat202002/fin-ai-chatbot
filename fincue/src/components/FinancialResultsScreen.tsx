  ...
  <Box flex={1} bg="gray.50" safeArea>
    <ScrollView>
      <VStack space={6} px={6} py={4}>
        ...
        {/* Recommendations Table */}
        <Box bg="white" rounded="2xl" shadow={2}>
          <VStack space={0}>
            <Box px={5} py={4} bg="primary.50" roundedTop="2xl">
              <Heading size="md" color="primary.700">
                Personalized Recommendations
              </Heading>
            </Box>
            ...
            {analysis.recommendations.map((rec: FinancialRecommendation, index: number) => (
              <Box key={index}>
                ...
                {index < analysis.recommendations.length - 1 && <Divider mx={5} />}
              </Box>
            ))}
          </VStack>
        </Box>
